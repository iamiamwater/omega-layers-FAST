    ...
    constructor()
    {
        ...
        this.multiplierUpgrades = [
            new MetaDynamicLayerUpgrade("Increase the Resource Multiplier",
                ...
                level => new Decimal(1),
                level => Decimal.pow(17, level.pow(game.restackLayer.upgradeTreeNames.resourceMultipliersLevelScaling.apply())).pow(this.getResourceMultiplierBoost())),
            new MetaDynamicLayerUpgrade("Increase the Resource Multiplier",
                level => Utils.createValueDilation(level.mul(3).mul(Decimal.pow(1.1, Decimal.max(0, level.sub(15)))), 0.001).floor().add(10),
                level => new Decimal(1),
                level => Decimal.pow(38, level.pow(game.restackLayer.upgradeTreeNames.resourceMultipliersLevelScaling.apply())).pow(this.getResourceMultiplierBoost())),
            new MetaDynamicLayerUpgrade("Increase the Resource Multiplier",
                level => Utils.createValueDilation(level.mul(12).mul(Decimal.pow(1.1, Decimal.max(0, level.sub(15)))), 0.001).floor().add(47),
                level => new Decimal(1),
                level => Decimal.pow(174, level.pow(game.restackLayer.upgradeTreeNames.resourceMultipliersLevelScaling.apply())).pow(this.getResourceMultiplierBoost())),
            new MetaDynamicLayerUpgrade("Increase the Resource Multiplier",
                level => Utils.createValueDilation(level.mul(48).mul(Decimal.pow(1.1, Decimal.max(0, level.sub(15)))), 0.001).floor().add(143),
                level => new Decimal(1),
                level => Decimal.pow(6561, level.pow(game.restackLayer.upgradeTreeNames.resourceMultipliersLevelScaling.apply())).pow(this.getResourceMultiplierBoost()))
        ];

        this.powerUpgrades = [
            new MetaDynamicLayerUpgrade("Increase the Resource Power by a crazy amount easy meta layer",
